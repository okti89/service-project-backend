...
def _service_context(service):
    ...
    return service_no, customer, address, appointment
...
def send_operational_alerts(now=None):
    """Send once-daily operational alerts for unassigned, overdue, and unpaid services."""
    ...
    for service in unassigned_services:
        service_no, customer, address, appointment = _service_context(service)
        message = (
    ...
    for service in overdue_services:
        ...
        if technician_user and technician_user.is_active:
            technician_message = (
                f"#{service_no} no'lu servisin planlanan saati geçti. "
                f"Lütfen servis durumunu güncelleyin.\n"
                f"Müşteri: {customer}\nAdres: {address}\nRandevu: {appointment}"
            )
